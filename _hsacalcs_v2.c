...
#=============================================================================================
  ...
  gcc -O3 -lm -fPIC -shared -I(directory with Python.h) -I(directory with numpy/arrayobject.h) -o _hsacalcs_v2.so _hsacalcs_v2.c

  For a desmond installation of python 2.5 (change path up to desmond directory, rest should be the same):
  
  gcc -O3 -lm -fPIC -shared -I /opt/schrodinger2014-2/mmshare-v26017/lib/Linux-x86_64/include/python2.7/ -I /opt/schrodinger2014-2/mmshare-v26017/lib/Linux-x86_64/lib/python2.7/site-packages/numpy/core/include/ -o _hsacalcs_v2.so _hsacalcs_v2.c
  
  For a default installation of python 2.5:
  
  gcc -O3 -lm -fPIC -shared -I/usr/local/include/python2.5 -I/usr/local/lib/python2.5/site-packages/numpy/core/include -o _hsacalcs_v2.so _hsacalcs_v2.c
...
float dist_mic(float x1, float x2, float x3, float y1, float y2, float y3, float b1, float b2, float b3) {
...
float dist(float x1, float x2, float x3, float y1, float y2, float y3) {
...
PyObject *_gistcalcs_nbr_E_ww( PyObject *self, PyObject *args)
    ...
    int i, m, o, p;
    int wat_O;
    float *b_x, *b_y, *b_z; // variables to store box info
    float *w1x, *w1y, *w1z, *h1x, *h1y, *h1z; // variables to store water molecule's coordinates (separately for O and H-atoms)
    double *w1c, *h1c, *w1_sig, *w1_eps; // variables to store water molecule's non-bonded params 
    float *w2x, *w2y, *w2z, *h2x, *h2y, *h2z; // variables to store water molecule's coordinates (separately for O and H-atoms)
    double *w2c, *h2c, *w2_sig, *w2_eps; // variables to store water molecule's non-bonded params 
    float dist6, dist12, d_oo, d_oh, d_hh; // distances 
    double comb_sig, comb_eps, aij_wat, bij_wat;
    ...
    w1x = PyArray_GETPTR2(coords, wat_O, 0); //
    w1y = PyArray_GETPTR2(coords, wat_O, 1); // obtain x, y, z
    w1z = PyArray_GETPTR2(coords, wat_O, 2);
    ...
    for (i = 0; i < m; i++) {
        ...
        w2x = PyArray_GETPTR2(coords, *target_O_at, 0); //
        w2y = PyArray_GETPTR2(coords, *target_O_at, 1); // obtain x, y, z
        w2z = PyArray_GETPTR2(coords, *target_O_at, 2);
        ...
        aij_wat = 4*comb_eps*pow(comb_sig, 12);
        bij_wat = -4*comb_eps*pow(comb_sig, 6);
        ...
        for (p = wat_O+1; p <= wat_O+2; p++){
            ...
            h1x = PyArray_GETPTR2(coords, p, 0); // use wat to get the correct x, y, z coordinates from coord array
            h1y = PyArray_GETPTR2(coords, p, 1); 
            h1z = PyArray_GETPTR2(coords, p, 2);
        ...
        for (o = *target_O_at+1; o <= *target_O_at+2; o++){
            ...
            h2x = PyArray_GETPTR2(coords, o, 0); // use wat to get the correct x, y, z coordinates from coord array
            h2y = PyArray_GETPTR2(coords, o, 1); 
            h2z = PyArray_GETPTR2(coords, o, 2);
...
static PyMethodDef _hsacalcs_v2_methods[] = {
    {
...
PyMODINIT_FUNC init_hsacalcs_v2() // init function has the same name as module, except with init prefix
{
    // we prodice name of the module, method table and a doc string
    Py_InitModule3("_hsacalcs_v2", _hsacalcs_v2_methods, "compute energies.\n");
